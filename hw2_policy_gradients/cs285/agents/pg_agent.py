...
class PGAgent(BaseAgent):
        ...
        if self.nn_baseline:
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    ## HINT: use terminals to handle edge cases. terminals[i] is 1 if the 
                    ## state is the last in its trajectory, and 0 otherwise.
                    if terminals[i] == 1:
                        advantages[i] = rews[i] - values[i]
                    else:
                        q_estimate = rews[i] + self.gamma * values[i+1]
                        delta = q_estimate - values[i]
    ...
    def _discounted_return(self, rewards):
        ...
        return [discounted for _ in range(len(rewards))]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        """
        N = len(rewards)
        discounted = [0 for i in range(N+1)]
        for i in reversed(range(N)):
            discounted[i] = rewards[i] + self.gamma*discounted[i+1]
