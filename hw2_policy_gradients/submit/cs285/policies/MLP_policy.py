...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self,
        ...
        if distributed:
            self.baseline = DataParallel(self.baseline, device_ids=[0,1])
            if discrete:
                self.logits_na = DataParallel(self.logits_na, device_ids=[0,1])
            else:
                self.mean_net = DataParallel(self.mean_net, device_ids=[0,1])
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        pred = self.get_action(observations, mode='train')
        loss = -(pred.log_prob(actions) * advantages).sum()
        ...
        if self.nn_baseline:
            ...
            target = (q_values - q_values.mean()) / q_values.std()
