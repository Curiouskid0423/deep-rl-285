...
class MBAgent(BaseAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        for i in range(self.ensemble_size):
            ...
            sampled_data = np.zeros(num_data, dtype=bool)
            selected_indices = np.random.randint(num_data, size=num_data_per_ens)
            sampled_data[selected_indices] = True

            observations = ob_no[sampled_data] # TODO(Q1)
            actions = ac_na[sampled_data] # TODO(Q1)
            next_observations = next_ob_no[sampled_data] # TODO(Q1)
