...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        data_statistics = {key: ptu.from_numpy(val) for key, val in data_statistics.items()}
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        delta_std, delta_mean = data_statistics['delta_std'], data_statistics['delta_mean']
        ...
        loss = self.loss(logits, target) # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
