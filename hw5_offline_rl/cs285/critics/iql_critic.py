...
class IQLCritic(BaseCritic):
    ...
    def __init__(self, hparams, optimizer_spec, **kwargs):
        ...
        ### YOUR CODE HERE ###
        self.v_net = network_initializer(self.ob_dim, 1).cuda()
    ...
    def expectile_loss(self, diff):
        ...
        """
        coeff = torch.where(diff > 0, self.iql_expectile, 1 - self.iql_expectile)
        return coeff * (diff**2)
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE HERE ###
        q_value = torch.gather(self.q_net_target(ob_no).detach(), 1, ac_na.unsqueeze(1)).squeeze(1)
        value = self.v_net(ob_no).squeeze(1)
        value_loss = self.expectile_loss(q_value - value).mean()
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ### YOUR CODE HERE ###
        q_value = torch.gather(self.q_net(ob_no), 1, ac_na.unsqueeze(1)).squeeze(1)
        value = self.v_net(next_ob_no).squeeze(1)
        loss = self.mse_loss(q_value, reward_n + self.gamma * value.detach())
