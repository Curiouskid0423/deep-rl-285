...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        # critic.
        qa_t_values = ptu.from_numpy(critic.qa_values(obs))
        if isinstance(action, int):
            return qa_t_values[:, action]
        else:
            return torch.gather(qa_t_values, 1, action.unsqueeze(1)).squeeze(1)
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        ac_na = ptu.from_numpy(ac_na).type(dtype=torch.int64)
        ...
        if self.agent_params['discrete']:
            for i in range(self.agent_params['ac_dim']):
                prob = torch.exp(dist.log_prob(ac_na[i]))
                next_q_val = self.get_qvals(self.exploitation_critic, ob_no, i)
                vals.append(prob * next_q_val)
        else:   
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            raw_expl_bonus = self.exploration_model.forward_np(ob_no)
            self.running_rnd_rew_std = self.rnd_gamma * self.running_rnd_rew_std \
                + (1-self.rnd_gamma) * raw_expl_bonus.std()
            expl_bonus = normalize(raw_expl_bonus, raw_expl_bonus.mean(), self.running_rnd_rew_std)
            ...
            # TODO: Uncomment these lines after completing awac
