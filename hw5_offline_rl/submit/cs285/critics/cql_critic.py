...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        """ Implement DQN Loss """
        # TODO: Similar to HW3 dqn_critic.py file
        qa_t_values = self.q_net(ob_no)
        q_t_values = torch.gather(qa_t_values, 1, ac_na.unsqueeze(1)).squeeze(1)
        best_actions = self.q_net(next_ob_no).argmax(dim=1)

        # Compute the Q-values from the target network (ground truth for supervised regression)
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        cql_loss = self.cql_alpha * (q_t_logsumexp - q_t_values).mean() + loss
